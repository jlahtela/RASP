...
-- Count files in a directory (for verification)
function file_ops.count_files_in_dir(path)
  ...
  local function scan_dir(dir)
    ...
    while true do
      ...
      scan_dir(file_ops.join_path(dir, subdir))
      i = i + 1
    end
  end
...
function file_ops.copy_directory(source, dest)
  ...
  if os_type == "windows" then
    ...
    -- Use cp -a on Linux/macOS (preserves permissions and timestamps)
    -- The "/." syntax ensures we copy contents, not the directory itself
    -- -a is better than -r as it preserves permissions, timestamps, and handles symlinks
